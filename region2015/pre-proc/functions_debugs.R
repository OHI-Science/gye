...
# @andres: Sustainability moved from constant 1.0 to LAYER(ao_soa)
AO = function(layers,
              year_max,
  ...
  r = na.omit(rename(dcast(layers_data, id_num ~ layer, value.var='val_num',
                           subset = .(layer %in% c('ao_access', 'ao_sao'))),
                     c('id_num'='region_id', 'ao_access'='access', 'ao_sao'='Sustainability'))); head(r); summary(r)
...
HAB = function(layers){
  ...
  d = d %>%
    filter(habitat %in% c('mangrove', 'soft_bottom')) %>% # c('coral','mangrove','rocky_reef', 'soft_bottom')) %>%
    mutate(
