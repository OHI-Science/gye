...
setwd('CW')
YEAR = 2015
database = 'cw_pathogen_trend.xlsx'
sheet = 'cw_pathogen_trend'
file = paste0('cw_pathogen_trend_gye', YEAR, '.csv')
...
DF %>% filter(! Provincia == 'Golfo de Guayaquil') %>% do({
  plot(.$year, .$value, main = paste('Pathogen', .$Provincia[1]), sub=paste(d$coef.Beta0[d$Provincia == .$Provincia[1]], '+', d$coef.year[d$Provincia == .$Provincia[1]]))
