...
# ecuador's region_id in the global assessment is 137
scores_global_ecuador = scores_v2014.3 %>%            
  filter(region_id == 137) %>%
  mutate(goal = as.character(goal))


## read in gye/region2015 scores         
...
scores_compare = scores_global_ecuador %>%   
  select(goal, 
         dimension,
         score_global = score) %>%         
